...
async function getMovies(params) {
    ...
    if (params.query) {
        url = `https://api.themoviedb.org/3/search/multi?api_key=${API_KEY}&query=${encodeURIComponent(params.query)}&language=${userLang}`;
    } else if (params.id) {
        url = `https://api.themoviedb.org/3/${params.type}/${params.id}?api_key=${API_KEY}&append_to_response=videos&language=${userLang}`;
    } else if (params.endpoint) {
        const joiner = params.endpoint.includes('?') ? '&' : '?';
        url = `https://api.themoviedb.org/3${params.endpoint}${joiner}api_key=${API_KEY}&language=${userLang}`;
    }
